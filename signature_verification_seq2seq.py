...
# encoding input data
def encoding(x, y, encoding_dict):
    index = encoding_dict.setdefault((x,y), len(encoding_dict) + 3)
    
    return index

# read all the pictures from a directory
def read_directory(path, signatures, timeStamp, target, encoding_dict):
        ...
        with open(path + '/' + file) as ifile:
            for line in ifile:
                if firstLine == 0:
                    ...
                    signature.append(encoding(x,y,encoding_dict))
...
def main():
    ...
    parameters = Parameters(2, 1, 0, 10, 300, 100, 100, 0.001, 5, 32, 0.001)

    batch_size = parameters.batch_size

    loss_track = []
    
    read_directory('./Task2/U1', signatures, timeStamp, target, encoding_dict)
